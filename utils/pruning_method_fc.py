...
def pruning_method_fc(model, layer_to_prune, pruning_amount, method):
    ...
    if method == 'L1norm':
        ...
        # print('number of layer to prune: ' + str(len(fc_layer_weights)))
    ...
    if method == 'DYJS_score': #filter_ranking_score
        # Load surgeon package
        surgeon = Surgeon(model)
        ...
        for i in range(len(fc_layer_weights)):
            ...
            filter_sum_negative_sort = sorted(filter_sum_negative.items(), key=lambda kv: kv[1])  # , reverse=True)
            ...
            # print(filter_sum_ranking_score_sort)
    ...
    if method == 'DYJS_step': # find_common_filters
        # Load surgeon package
        surgeon = Surgeon(model)
        ...
        for i in range(len(fc_layer_weights)):
            ...
            buff = []
            similar_array = []
            # for j in range(pruning_amount[i], num_filters, pruning_amount[i]):
    ...
    elif method == 'DYJS_step_gm': # geometric_median_conv
        # Load surgeon package
        surgeon = Surgeon(model)
