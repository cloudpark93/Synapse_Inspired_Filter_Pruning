...
def _smallest_size_at_least(height, width, smallest_side):
  ...
  new_height = tf.compat.v1.to_int32(tf.compat.v1.rint(height * scale))
  new_width = tf.compat.v1.to_int32(tf.compat.v1.rint(width * scale))
...
def preprocess_for_train(image,
  ...
  with tf.compat.v1.name_scope(scope, 'distort_image', [image, height, width, bbox]):
    ...
    rotated_image = tfa.image.rotate(image, math.radians(angle), interpolation='nearest')
    #tf.compat.v1.contrib.image.rotate(image, math.radians(angle), interpolation='BILINEAR')
